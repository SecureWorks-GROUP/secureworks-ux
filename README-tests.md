...
19. The Ops Dash make-safe job detail states the canonical board stage instead of re-deriving one from substatus, and withholds every forward-move button on a terminal or unconfirmed stage; the same spec (`tests/e2e/ops-makesafe-ui-truth.spec.js`) covers the canonical family tag, the "Family not determined" state, the "No pack drafted" Docs Ready marker and the card-face builder links.
