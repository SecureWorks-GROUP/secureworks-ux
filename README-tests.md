...
This suite is the SecureWorks Group copyable pattern for static HTML tools. It runs on every pull request and covers the Trade App's and Ops Dash's highest-value read-only flows in Chromium, plus Ops Dash calendar coverage: a pure-node unit spec for the working-day date math, a real-pointer drag regression spec, and a rendered-cascade spec for the Schedule view's day-cell rules.
...
26. Any Docs Ready card with a canonical drafted pack opens the review/send overlay (queue membership is not the door), an assessment with no pack keeps "No pack drafted" with no Review button, and Hours & wording edits preview on the pane, lock APPROVE AND SEND, and die with their docket revision. `tests/e2e/ops-ses-loop-overlay.spec.js` guards this; see `AGENTS.md` for the authoritative overlay contract and `docs/evidence/ses-loop-ux-overlay-v1/` for the proof shots.
27. The Ops Dash background board refresh cannot repaint or tear down an open full-page make-safe job detail: the five-minute jobs timer's background `loadJobs()` stands down while `#jobDetailView` is active (including a response that arrives after the detail opens mid-fetch), and every late make-safe enricher gates its re-render on `makesafeBoardCanRepaint()`, while the auth-state, `ai_alerts` realtime, and comms-poll handlers are proven never to touch the board at all; the periodic refresh still runs and an explicit press still loads when no detail is open. `scripts/test-ops-job-detail-autorefresh.js` executes the real shipped `ops.html` callbacks in a VM harness and runs before Playwright through `npm run test:e2e`.
28. Allocated trades see quote numbers, job writing, compact WO line items, and every job video on the Scope tab (and Files / Photos / the make-safe report header), never `$` / rates / totals on quote-pack and compact WO surfaces unless `canSeeFullPricing()` (office / tier ≥ 3). The full priced MakeSafe / supplier work-order PDF is office-only: trades get no Open full WO link, iframe, Files WO card (including noncanonical `WO` aliases on the second Files list), photo-gallery thumb, external/source link, or a `<video>` of that PDF — even when the row claims to be video or lands in `data.media` as a photo. Work Order tab Cost Breakdown / Crew Charges use the same office SOW gate (`tradeCanSeeSowPricing()`), not `canSeePricing()` / tier ≥ 2. Every direct `data.media` renderer applies visibility + priced-WO filtering. Trade-visible note bodies use `escSowText()`, including the Quick Notes overlay (`detail_json` text/note/note_text, string JSON parsed). External-link normalise keeps kind/type/file/PDF metadata so a typed `work_order` / `supplier_work_order` signed URL with a generic label is still office-only. Every typed metadata field is checked before video heuristics, so `type: video` cannot hide a WO PDF. MakeSafe builder-email URLs use the same office-only WO href filter. Free-text redaction covers cue-word totals, `8.8k`, `/day` rates, WO/client/MakeSafe prose, video labels, Notes/Log, and Quick Notes. Media marked `visible_to_trades: false` is hidden on every source. A walkthrough flagged in `scope_json` with no playable URL is an honest empty, not an omitted section. `scripts/test-trade-scope-media.js` extracts the shipped `// <trade-scope-media>` block; `tests/e2e/trade-scope-videos-pricing.spec.js` covers the rendered tabs and report header.
...
- `tests/e2e/ops-calendar-day-rules.spec.js` — the Schedule view's day cells must be ruled off from the row below, so a job bar or an "away" strip reads against a grid instead of a flat field. It renders the shipped `renderScheduleView` against a populated August 2026 fixture (multi-day patio/fencing/decking bars plus crew leave) and asserts the RENDERED cascade and box metrics, never the stylesheet text: every interior week row's cells carry a 1px bottom rule in the `--sw-border` token at the same weight and colour as the vertical rule they meet, the last row carries none (its cells sit directly inside the grid's own 2px border, where a rule would separate nothing and thicken that edge), and the 2w/1w ranges share the same cell class and the same rule. The rest is the no-reflow contract that makes this safe on the crew's live iPads: `box-sizing: border-box` means the rule consumes content box, so every cell is still 90px tall, rows stack flush, grid height is unchanged, every bar still has a real box, and the weekend/out-of-month/today tints are untouched. Before/after screenshots and the offline harness that produced them (its own local server, every off-origin request aborted) live in `docs/evidence/ops-calendar-day-borders-2026-09-04/` and `scripts/cal-month-border-shot.js`. The Crew (swimlane) view is deliberately out of scope: it already rules its cells through the grid container's `gap: 1px` over a `var(--sw-border)` background.
