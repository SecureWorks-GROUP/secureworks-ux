...
26. Any Docs Ready card with a canonical drafted pack opens the review/send overlay (queue membership is not the door), an assessment with no pack keeps "No pack drafted" with no Review button, and Hours & wording edits preview on the pane, lock the send stamp, and die with their docket revision. On a sendable pack the stamp word is SEND IT (send-only) or APPROVE AND SEND (invoice still armed); the click always runs `sesApproveAndSend`. An in-flight Northam-class release (approved / dispatching / partially released) keeps SEND IT enabled even when `send_it.enabled` is false, and the press resumes `execute_ses_release_revision` on that revision — it does not prepare a new one. `tests/e2e/ops-ses-loop-overlay.spec.js` guards this; see `AGENTS.md` for the authoritative overlay contract and `docs/evidence/ses-loop-ux-overlay-v1/` for the proof shots.
