...
11. A managed fencing lead (`role: lead_installer`, `managed_verticals: ["fencing"]`) sees every fencing job without becoming an ops manager: the Board opens on the fencing vertical with his own, another crew's, and open-pool work and no make-safe or other-vertical cards; My Jobs defaults to Everyone and Mine drops other-crew plus open rows; the calendar consumes `trade-calendar.v1` as fencing Everyone and Mine refetches with `mode=mine`. Every read is a GET carrying the bearer token. Another crew's job detail is view-only (banner, no crew actions, no note/photo/comms inputs) and a forced clock write is refused without reaching the network or the offline queue, while his own fencing job keeps its crew actions.
12. Optional dedicated accounts prove that the real Supabase password login still works.
