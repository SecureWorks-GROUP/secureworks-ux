...
Captured with `chrome-devtools-axi` against the real `ops.html#performance` host through `scripts/sales-performance-preview.py`. The offline envelope is derived from the supplied 14 September example, with all customer names removed. No live systems or credentials were used.
...
Screenshots: [desktop light](desktop-light.png), [desktop dark](desktop-dark.png), [phone light](phone-light.png), [phone dark](phone-dark.png). These are viewport captures: the host's offscreen translated SMS drawer appears spuriously in full-page captures, so those were replaced with viewport captures. The drawer was not opened or changed.
