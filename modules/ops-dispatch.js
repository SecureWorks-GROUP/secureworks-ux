...
(function (root) {
  ...
  function mount(element, options = {}) {
    ...
    element.addEventListener('input', event => {
      ...
      if (target.closest('[data-form]')?.dataset.form === form?.kind && form && !target.dataset.editor) { const data = new FormData(target.closest('form')); Object.assign(form.values, Object.fromEntries(data)); if (form.kind === 'order' || form.kind === 'movement') form.values.requirement_ids = data.getAll('requirement_id'); }
    });
    ...
    element.addEventListener('click', async event => {
      ...
      try {
        ...
        if (action === 'tab') { tab = target.dataset.id; render(); if (tab === 'email' && id) await core.execution(id); }
        ...
        if (action === 'approve-draft') {
          ...
          if (core.editor(id, `draft:${draft.id}`) || draft.review?.content_hash !== draft.content_hash || draft.review?.source_version !== record.source_version) throw new Error('Save and review the current exact draft before approving it.');
        ...
        if (action === 'execute-draft') { const draft = draftValue(record); if (core.editor(id, `draft:${draft.id}`) || !draft.approval || draft.approval.content_hash !== draft.content_hash || draft.approval.source_version !== record.source_version) throw new Error('Current exact approval is required; unsaved changes cannot be sent.'); await core.executeDraft(id, draft.id, draft.approval.id); }
