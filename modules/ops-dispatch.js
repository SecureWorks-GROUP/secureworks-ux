...
(function (root) {
  ...
  function mount(element, options = {}) {
    ...
    let dates = root.DispatchCore.week(options.now), queue = 'current', trade = 'all', query = '', workflow = 'all', tab = 'email', selectedGroup = 'all';
    let form = null, contextJob = null, message = '', mailResults = null, mailGeneration = 0, selectedMail = null, canonicalMailGeneration = 0, selectionGeneration = 0;
    ...
    function currentUI() {
      const id = core.state.selectedId;
      if (!uiByJob.has(id)) uiByJob.set(id, { draftId: null, draftReview: null, mailPoId: '', canonicalMail: null, workflowRefresh: null, mailSearch: { scope: 'job', search: '' }, disclosures: {}, focus: null });
    ...
    function formHTML(record) {
      ...
      if (form.kind === 'requirement') {
        for (const [field, fallback] of Object.entries({ phase: 'installation', destination: 'site', owner: 'Shaun' })) if (!Object.hasOwn(values, field)) values[field] = fallback;
      }
    ...
    function groundingHTML(record) {
      const sources = array(record.documents).concat(array(record.media));
      const missing = [];
      if (!record.job.scope_json) missing.push('stored scope');
      if (!record.job.pricing_json) missing.push('accepted quote / pricing');
      if (!sources.length) missing.push('video or document attachments');
      return `<section class="dp-grounding" data-workshop="grounding"><h3>Job grounding</h3><p class="dp-small">What needs ordering is in the groups below. Extraction is assistive only where it exists. Unknowns stay unknown — enter them manually. An AI plan is not required to use this workshop.</p>${record.job.scope_json ? `<details open class="dp-source" data-disclosure="workshop-scope"><summary>Stored job scope</summary>${structuredSource(record.job.scope_json, ['scope'])}</details>` : '<p class="dp-small dp-muted">Stored scope unavailable. Add requirements by hand.</p>'}${record.job.pricing_json ? `<details class="dp-source" data-disclosure="workshop-pricing"><summary>Stored quote / pricing</summary>${structuredSource(record.job.pricing_json, ['pricing'])}</details>` : '<p class="dp-small dp-muted">Stored quote unavailable.</p>'}${sources.length ? sources.map(source => `<div class="dp-source"><strong>${esc(source.name || source.file_name || source.title || source.type)}</strong><p>${esc(source.source || source.type)} · ${esc(source.revision || source.updated_at || 'Revision unknown')}</p>${safeLink(source.url || source.file_url || source.pdf_url || source.storage_url) ? `<a href="${safeLink(source.url || source.file_url || source.pdf_url || source.storage_url)}" target="_blank" rel="noopener noreferrer">Open original</a>` : '<p>Original attachment unavailable</p>'}</div>`).join('') : ''}${missing.length ? `<p class="dp-notice">Missing extraction: ${esc(missing.join(', '))}. Manual workshop still works.</p>` : ''}</section>`;
    }
    function workspaceHTML(record) {
      ...
      return `<div class="dp-workhead" data-workshop="job"><div class="dp-row"><div class="dp-grow"><h2>${esc(record.job.job_number)} <span class="dp-muted">${esc(record.job.client_name)}</span></h2><p>${esc(record.job.site_address || 'Site address unavailable')} · ${esc(record.job.work_type || record.job.type || 'Work type unknown')}</p></div><span class="dp-tag ${record.job.eligibility?.state !== 'accepted' ? 'dp-tag-held' : ''}">${esc(label(record.job.eligibility?.state || 'Eligibility unresolved'))}</span></div><p>${stale ? 'Requirements review incomplete or sources changed' : 'Current source set reviewed'} · Version ${record.version}</p></div>${groundingHTML(record)}${calendarHTML(core, dates)}
        ...
        <details class="dp-assessment" data-disclosure="optional-assessment" data-workshop="optional-ai"><summary>Optional AI assessment (future)</summary><p class="dp-small">This workshop does not require an AI plan or registered assessor. Suggestions below are optional and separately labelled.</p><div class="dp-row"><h3 class="dp-grow">Assess current evidence</h3>${button('assess', 'Assess current evidence', busy() ? 'disabled' : '')}</div>${record.assessment ? `<p>${esc(record.assessment.summary || record.assessment.next_action || 'Assessment recorded')}</p><ul>${array(record.assessment.obligations || record.assessment.findings || record.assessment.blockers).map(finding => `<li>${esc(typeof finding === 'string' ? finding : finding.next_action || finding.message || finding.description)} ${esc(finding.owner || '')}</li>`).join('')}</ul><p class="dp-small">Source ${esc(record.assessment.source_version)} ${record.assessment.stale || record.assessment.source_version !== record.source_version ? '· Stale — reassess current evidence' : ''}</p>` : '<p>Manual reconcile of requirements, linked supply and movements is enough to work the job.</p>'}${assessmentStatusHTML()}</details>`;
    }
    ...
    function render() {
      ...
      if (contextJob !== state.selectedId) { if (form && contextJob) formsByJob.set(contextJob, form); contextJob = state.selectedId; selectedGroup = 'all'; form = formsByJob.get(contextJob) || null; selectedMail = null; mailResults = null; mailGeneration++; canonicalMailGeneration++; selectionGeneration++; message = ''; }
      ...
      element.innerHTML = `<header class="dp-header"><div class="dp-row"><div><h1>Dispatch</h1><p>Job workshop · what needs ordering and the relevant history. AI suggestions are future scope.</p></div>${root.OpsHowItWorks ? root.OpsHowItWorks.iconButton('dispatch') : ''}</div><div class="dp-tools">${button('choose-job', 'Choose job', 'class="dp-mobile-only"')}${button('refresh', 'Refresh evidence')}${root.OpsWorkflowRefresh ? root.OpsWorkflowRefresh.html('dispatch', currentUI().workflowRefresh) : ''}</div></header><div class="dp-layout"><aside class="dp-panel dp-queue"><div class="dp-pad"><h3>Accepted jobs</h3><p class="dp-small dp-muted">Current material is acceptance-evidenced work that is not complete, invoiced, final payment or review-collection history. No PO is not proof nothing was ordered.</p><div class="dp-trades">${[['current', 'Current material'], ['acceptance-review', 'Acceptance review'], ['historical', 'Historical']].map(([value, name]) => `<button data-action="queue" data-id="${esc(value)}" aria-pressed="${queue === value}">${esc(name)}</button>`).join('')}</div><div class="dp-trades">${[['all', 'All work types'], ...types].map(([value, name]) => `<button data-action="trade" data-id="${esc(value)}" aria-pressed="${trade === value}">${esc(name)}</button>`).join('')}</div><input class="dp-search" data-filter="search" aria-label="Search Dispatch material queue" value="${esc(query)}" placeholder="Job, customer or address"><select class="dp-filter" data-filter="workflow" aria-label="Next action"><option value="all">All next actions</option>${actions.map(value => `<option value="${esc(value)}" ${workflow === value ? 'selected' : ''}>${esc(label(value))}</option>`).join('')}</select></div><div class="dp-job-list">${jobs.map(item => `<button class="dp-job" data-action="select" data-id="${esc(item.id)}" aria-pressed="${item.id === state.selectedId}"><span>${esc(item.job_number)} · ${esc(item.work_type)}</span><strong>${esc(item.client_name || 'Customer unavailable')}</strong><span>${esc(item.site_address)}</span><span>${esc(label(item.next_action || 'Review materials'))}</span></button>`).join('') || `<div class="dp-pad dp-muted">${state.jobsLoading ? 'Reading Dispatch work…' : 'No matching captured jobs in this queue.'}</div>`}</div><div class="dp-footer">${esc(queueFooter(state))}${state.error ? `<p class="dp-conflict">${esc(state.error)}</p>` : ''}</div></aside><main class="dp-panel dp-workspace"><p class="dp-notice" role="status">${esc(evidenceStamp)} · ${esc(evidenceStatus)}. Evidence refresh is a source reload, not an AI assessment.</p>${state.selectedId && !jobs.some(item => item.id === state.selectedId) ? `<div class="dp-notice">Selected job is outside this filter. ${button('reset-filters', 'Show selected job · reset filters')}</div>` : ''}${error ? `<div class="dp-notice dp-error" role="alert">${esc(error)} ${!pending ? button('reload-job', 'Retry job') : ''}</div>` : ''}${pending && !pending.running ? `<div class="dp-notice dp-error" role="alert">${pending.conflict ? 'Saved sources changed. Reload and review before saving again.' : 'The previous save outcome is unconfirmed. Its exact request remains available for recovery.'} ${button(pending.conflict ? 'resolve-conflict' : 'retry-write', pending.conflict ? 'Reload changed evidence — keep edits' : 'Retry same request')}</div>` : ''}${pending?.running ? '<p class="dp-notice" role="status">Saving and checking this job…</p>' : ''}${message ? `<p class="dp-notice" role="status">${esc(message)}</p>` : ''}${record ? workspaceHTML(record) : `<div class="dp-blank">${state.selectedId ? 'Reading job workshop…' : 'Choose a job to open its workshop: grounding, materials and relevant history.'}</div>`}</main><aside class="dp-panel dp-context"><div class="dp-tabs" role="tablist" aria-label="Job history and notes">${['email', 'notes', 'scope'].map(value => `<button role="tab" data-action="tab" data-id="${value}" aria-selected="${tab === value}">${{ scope: 'Original evidence', notes: 'Working notes', email: 'Job history' }[value]}</button>`).join('')}</div><div class="dp-pad">${record ? tab === 'scope' ? sourceHTML(record) : tab === 'notes' ? notesHTML(record) : emailHTML(record) : '<p class="dp-muted">Select a job to see whole-job history. Narrow with the PO filter if needed.</p>'}</div></aside></div>`;
    ...
    listen('submit', async event => {
      ...
      } catch (error) { if (['dispatch_identity_changed', 'dispatch_selection_changed'].includes(error.code)) return; if (core.state.selectedId === id) message = error.message; }
      render();
    });
    listen('click', async event => {
      ...
      try {
        ...
        if (action === 'select') {
          await core.select(target.dataset.id);
          if (innerWidth <= 700) element.querySelector('.dp-workspace')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
          if (tab === 'email') loadCanonicalMail();
          return;
        }
        ...
        if (action === 'workflow-refresh') {
          ...
          const owner = identityGuard(), generation = selectionGeneration, ui = currentUI();
          const assertIdentity = () => {
            owner();
            if (destroyed || generation !== selectionGeneration || core.state.selectedId !== id) {
              const error = new Error('Dispatch job selection changed.');
              error.code = 'dispatch_selection_changed';
              throw error;
            }
          };
          assertIdentity();
          const run = await root.OpsWorkflowRefresh.start('dispatch', { job_id: id }, { assertIdentity });
          assertIdentity();
          if (run.lease_token) throw new Error('Refresh readback exposed a lease token.');
          ui.workflowRefresh = run;
          render();
          return;
        }
