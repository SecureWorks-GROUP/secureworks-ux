...
(function (root) {
  ...
  function plainMail(mail) {
    if (mail.body_text || mail.body || mail.snippet) return mail.body_text || mail.body || mail.snippet;
    if (!mail.body_html) return 'Original message body unavailable';
  ...
  function structuredSource(value) {
    ...
    return `<dl class="dp-source-fields">${Object.entries(value).map(([key, item]) => `<dt>${esc(label(key))}</dt><dd>${item && typeof item === 'object' ? `<details><summary>${Array.isArray(item) ? item.length + ' recorded values' : 'Recorded details'}</summary>${structuredSource(item)}</details>` : esc(item ?? 'Unknown')}</dd>`).join('')}</dl>`;
  }
  let shared;
  function controller() {
  ...
  function calendarHTML(core, dates) {
    ...
    return `<section class="dp-calendar" aria-label="Shared work calendar"><div class="dp-row"><strong class="dp-grow">${esc(dateLabel(dates[0]))} – ${esc(dateLabel(dates[6]))} · Perth</strong><button data-action="week-back" aria-label="Previous week">‹</button><button data-action="week-next" aria-label="Next week">›</button>${Object.keys(layers).map(layer => `<label><input type="checkbox" data-layer="${layer}" ${layers[layer] ? 'checked' : ''}>${label(layer)}</label>`).join('')}</div>
      ...
      ${array(calendar.undated).length ? `<details><summary class="dp-small">Undated requirements & movements</summary>${calendar.undated.map(event => `<button class="dp-email-result" data-action="event" data-job="${esc(event.job_id)}" data-id="${esc(event.id)}">${esc(event.job_number)} · ${esc(event.title)}</button>`).join('')}</details>` : ''}</section>`;
  }
  function mount(element, options = {}) {
    ...
    function currentUI() {
      ...
      if (!uiByJob.has(id)) uiByJob.set(id, { draftId: null, draftReview: null });
    ...
    function sourceHTML(record) {
      ...
      return `<h3>Scope & quote</h3><p class="dp-small dp-muted">Original evidence, before material decisions.</p>${record.job.scope_json ? `<details open class="dp-source"><summary>Stored job scope</summary>${structuredSource(record.job.scope_json)}</details>` : '<p class="dp-small dp-muted">Stored scope unavailable.</p>'}${record.job.pricing_json ? `<details class="dp-source"><summary>Stored quote / pricing inputs</summary>${structuredSource(record.job.pricing_json)}</details>` : '<p class="dp-small dp-muted">Stored pricing inputs unavailable.</p>'}${sources.length ? sources.map(source => `<div class="dp-source"><strong>${esc(source.name || source.file_name || source.title || source.type)}</strong><p>${esc(source.source || source.type)} · ${esc(source.revision || source.updated_at || 'Revision unknown')}</p><p>${source.accepted_at ? 'Accepted ' + esc(source.accepted_at) : 'Acceptance unverified'}${source.superseded_at ? ' · Superseded ' + esc(source.superseded_at) : ''}</p>${safeLink(source.url || source.file_url || source.pdf_url || source.storage_url) ? `<a href="${safeLink(source.url || source.file_url || source.pdf_url || source.storage_url)}" target="_blank" rel="noopener noreferrer">Open original</a>` : '<p>Original attachment unavailable</p>'}</div>`).join('') : '<div class="dp-source">No scope, quote or media was returned. Requirements remain incomplete until the source is established.</div>'}
        <details class="dp-divider"><summary>Current system context</summary>${array(record.context_facts).map(fact => `<div class="dp-source"><strong>${esc(fact.fact_type || fact.type || fact.kind)}</strong><p>${esc(fact.content || fact.summary || fact.fact_text || JSON.stringify(fact))}</p></div>`).join('') || '<p class="dp-small">No current context facts returned.</p>'}${button('review-context', 'Review current context evidence')}</details><details class="dp-divider"><summary>Source coverage & review</summary><p class="dp-small">${esc(JSON.stringify(record.coverage || {}))}</p><p class="dp-small">Current source: ${esc(record.source_version)}<br>Reviewed source: ${esc(record.reviewed_source_version || 'Not reviewed')}</p></details>`;
    }
    ...
    function executionHTML(record, draft) {
      ...
      const approved = approval && approval.content_hash === draft.content_hash && approval.source_version === record.source_version && !core.editor(record.job.id, `draft:${draft.id}`);
      return `<div class="dp-divider"><h3>Communications approval & delivery</h3><p class="dp-authority">${capabilities.release_hold !== false ? 'Sending is held by the server. No provider contact is authorised.' : 'Release enabled; exact communications approval is still required.'}${draft.thread_id ? ' Captured-thread sending is not available on this transport.' : ''}</p>${execution?.error ? `<p class="dp-conflict">Status unavailable: ${esc(execution.error)}</p>` : ''}
        <form data-form="approval" class="dp-editor"><label><span><input type="checkbox" name="communications_approved">Approve this exact message and attachments</span></label>${draft.po_id ? '<label><span><input type="checkbox" name="purchase_approved">Separate purchase approval for this exact PO</span></label>' : ''}<div class="dp-tools">${button('approve-draft', 'Record exact approval', !capabilities.approval_enabled || !draft.review || core.editor(record.job.id, `draft:${draft.id}`) ? 'disabled' : '')}${button('execute-draft', capabilities.release_hold !== false ? 'Send held' : 'Send approved draft', capabilities.release_hold !== false || !approved || draft.thread_id || actions.some(action => action.status !== 'held') ? 'disabled' : '')}${button('execution-status', 'Refresh action status')}</div></form>
        ${actions.map(action => `<div class="dp-source"><strong>${esc(label(action.status))}</strong><p>${action.status === 'accepted_not_delivered' ? 'Provider accepted the request; delivery is not established.' : action.status === 'outcome_unknown' ? 'Outcome uncertain. Read back the provider record before any recovery; no blind resend.' : 'Action state from the server.'}</p><details><summary>Action receipt</summary><div class="dp-exact-review">${esc(JSON.stringify(action.receipt || {}, null, 2))}</div></details>${button('execution-readback', 'Read back provider receipt', `data-id="${esc(action.approval_id)}"`)}</div>`).join('')}
      </div>`;
    }
    function emailHTML(record) {
      ...
      if (!draft) return `<h3>Job email</h3><div class="dp-tools">${button('new-draft', 'Compose email', 'class="dp-primary"')}</div>${array(record.drafts).map(item => `<button class="dp-email-result" data-action="open-draft" data-id="${esc(item.id)}">${esc(item.subject || 'Untitled draft')}<small>Saved Dispatch draft · ${esc(item.status || 'not sent')}</small></button>`).join('')}<div class="dp-divider">${history}${viewed}</div>`;
    ...
    function workspaceHTML(record) {
      ...
      const stale = record.reviewed_source_version !== record.source_version;
    ...
    function render() {
      ...
      const jobs = root.DispatchCore.filteredJobs(state.jobs, { trade, query, workflow });
      const actions = [...new Set(state.jobs.map(item => item.next_action).filter(Boolean))];
      ...
      element.innerHTML = `<header class="dp-header"><div><h1>Dispatch</h1><p>Materials ready for work · accepted work, order groups and movement</p></div><div class="dp-tools">${button('choose-job', 'Choose job', 'class="dp-mobile-only"')}${button('refresh', 'Refresh evidence')}</div></header><div class="dp-layout"><aside class="dp-panel dp-queue"><div class="dp-pad"><h3>Accepted work</h3><p class="dp-small dp-muted">Unresolved eligibility stays visible</p><div class="dp-trades">${['all', 'fencing', 'patio'].map(value => `<button data-action="trade" data-id="${value}" aria-pressed="${trade === value}">${value === 'all' ? 'All' : value === 'patio' ? 'Patios' : 'Fencing'}</button>`).join('')}</div><input class="dp-search" data-filter="search" aria-label="Search accepted work" value="${esc(query)}" placeholder="Job, customer or address"><select class="dp-filter" data-filter="workflow" aria-label="Next action"><option value="all">All next actions</option>${actions.map(value => `<option value="${esc(value)}" ${workflow === value ? 'selected' : ''}>${esc(label(value))}</option>`).join('')}</select></div><div class="dp-job-list">${jobs.map(item => `<button class="dp-job" data-action="select" data-id="${esc(item.id)}" aria-pressed="${item.id === state.selectedId}"><span>${esc(item.job_number)} · ${esc(item.work_type)}</span><strong>${esc(item.client_name || 'Customer unavailable')}</strong><span>${esc(item.site_address)}</span><span>${esc(label(item.next_action || 'Needs assessment'))}</span></button>`).join('') || `<div class="dp-pad dp-muted">${state.jobsLoading ? 'Reading accepted work…' : 'No matching captured jobs.'}</div>`}</div><div class="dp-footer">${state.jobs.length} captured · ${state.jobsLoading ? 'Reading remaining pages' : state.coverage?.complete ? 'Population scan complete' : 'Coverage incomplete'}${state.error ? `<p class="dp-conflict">${esc(state.error)}</p>` : ''}</div></aside><main class="dp-panel dp-workspace">${state.selectedId && !jobs.some(item => item.id === state.selectedId) ? `<div class="dp-notice">Selected job is outside this filter. ${button('reset-filters', 'Show selected job · reset filters')}</div>` : ''}${error ? `<div class="dp-notice dp-error" role="alert">${esc(error)} ${pending ? button(pending.conflict ? 'resolve-conflict' : 'retry-write', pending.conflict ? 'Reload changed evidence — keep edits' : 'Retry same request') : button('reload-job', 'Retry job')}</div>` : ''}${pending?.running ? '<p class="dp-notice" role="status">Saving and checking this job…</p>' : ''}${message ? `<p class="dp-notice" role="status">${esc(message)}</p>` : ''}${record ? workspaceHTML(record) : `<div class="dp-blank">${state.selectedId ? 'Reading job evidence…' : 'Choose captured work to establish its material requirements.'}</div>`}</main><aside class="dp-panel dp-context"><div class="dp-tabs" role="tablist" aria-label="Job context">${['scope', 'notes', 'email'].map(value => `<button role="tab" data-action="tab" data-id="${value}" aria-selected="${tab === value}">${{ scope: 'Scope & quote', notes: 'Working notes', email: 'Email' }[value]}</button>`).join('')}</div><div class="dp-pad">${record ? tab === 'scope' ? sourceHTML(record) : tab === 'notes' ? notesHTML(record) : emailHTML(record) : '<p class="dp-muted">Job context appears here.</p>'}</div></aside></div>`;
      if (focus) { const input = [...element.querySelectorAll('[data-editor]')].find(node => node.name === focus.name && node.dataset.editor === focus.editor); input?.focus(); try { input?.setSelectionRange(focus.start, focus.end); } catch (_) {} }
    }
    function captureDraft(formElement, record) {
      ...
      const value = { ...current, sender: data.get('sender'), to: emails(data.get('to')), cc: emails(data.get('cc')), subject: data.get('subject'), body: data.get('body'), proposed_delivery_at: data.get('proposed_delivery_at') || null,
      ...
      core.edit(record.job.id, `draft:${value.id}`, value); currentUI().draftReview = null;
    ...
    function newDraft(record, extra = {}) {
      ...
      core.edit(record.job.id, `draft:${id}`, { id, sender: '', to: [], cc: [], subject: `Materials · ${record.job.job_number}`, body: '', attachments: [], ...extra }); tab = 'email'; render();
    ...
    element.addEventListener('input', event => {
      ...
      if (target.closest('[data-form]')?.dataset.form === form?.kind && form && !target.dataset.editor) { const data = new FormData(target.closest('form')); Object.assign(form.values, Object.fromEntries(data)); if (form.kind === 'order') form.values.requirement_ids = data.getAll('requirement_id'); }
    });
    ...
    element.addEventListener('submit', async event => {
      event.preventDefault(); const type = event.target.dataset.form, record = job(); if (!record) return;
      ...
      try {
        ...
        if (type === 'requirement') {
          ...
          await save(form.noteId ? 'note_promote' : 'requirement_upsert', form.noteId ? { id: form.noteId, requirement } : requirement, id);
        }
        ...
        if (type === 'transfer') await save('receipt_transfer', { id: form.values.id, location: data.location, evidence: data.evidence }, id);
        if (type === 'movement') await save('movement_upsert', { id: form.values.id || core.uuid(), ...data, date: data.date || null, time: data.time || null, requirement_ids: form.values.requirement_ids || [] }, id);
        if (submittedForm && JSON.stringify(submittedForm.values) !== submittedFields) { if (submittedForm.noteId) delete submittedForm.noteId; formsByJob.set(id, submittedForm); if (core.state.selectedId === id) message = 'Earlier values saved. New edits remain for review.'; } else { formsByJob.delete(id); if (core.state.selectedId === id) form = null; }
    ...
    element.addEventListener('click', async event => {
      ...
      try {
        ...
        if (action === 'reset-filters') { trade = 'all'; workflow = 'all'; query = ''; render(); return; }
        ...
        if (action === 'refresh') { await Promise.all([core.list(), core.calendar(dates[0], dates[6]), id ? core.load(id).catch(() => {}) : null]); return; }
        ...
        if (action === 'tab') { tab = target.dataset.id; if (tab === 'email' && id) await core.execution(id); }
        ...
        if (action === 'edit-order') { const order = record.order_drafts.find(item => item.id === target.dataset.id); form = { kind: 'order', jobId: id, values: { ...order, requirement_ids: array(order.line_items).map(line => line.dispatch_requirement_id) } }; }
        ...
        if (action === 'transfer-receipt') form = { kind: 'transfer', jobId: id, values: { id: target.dataset.id } };
        ...
        if (action === 'new-draft') return newDraft(record);
        ...
        if (action === 'approve-draft') {
          ...
          if (!data.has('communications_approved') || (draft.po_id && !data.has('purchase_approved'))) throw new Error('Explicit communications approval and any separate purchase approval are required.');
        ...
        if (action === 'execute-draft') { const draft = draftValue(record); if (!draft.approval) throw new Error('Exact approval is required.'); await core.executeDraft(id, draft.id, draft.approval.id); }
        ...
        if (action === 'save-draft' || action === 'review-draft') {
          ...
          if (action === 'review-draft' && JSON.stringify(canonicalDraft?.attachments || []) !== JSON.stringify(value.attachments)) { message = 'Attachment bytes resolved. Inspect the returned exact files and hashes, then review the draft again.'; render(); return; }
        ...
        if (action === 'open-linked-mail') { const generation = ++mailGeneration; const result = await core.communications({ job_id: target.dataset.job, scope: 'job' }); if (generation === mailGeneration && core.state.selectedId === id) { selectedMail = array(result.records).find(mail => mail.id === target.dataset.id); if (!selectedMail) message = 'Original reference is outside this captured page or unavailable. Search the original job; the reference remains linked.'; } }
        ...
        if (action === 'reply-mail') return newDraft(record, { thread_id: selectedMail.thread_id || null, sender: selectedMail.mailbox || selectedMail.mailbox_email || '', to: selectedMail.sender || selectedMail.from_email ? [selectedMail.sender || selectedMail.from_email] : [], subject: `Re: ${selectedMail.subject || ''}` });
      } catch (error) { if (core.state.selectedId === id) message = error.message; }
      render();
    });
    const unsubscribe = core.subscribe(render); render();
    return { core, render, load: () => Promise.all([core.list(), core.calendar(dates[0], dates[6])]), destroy: unsubscribe };
  }
  let instance, mainConnected = false;
  function mainDeliveries(deliveries, range) {
    ...
    if (core.state.layers.materials) array(deliveries).forEach(item => {
      ...
      rows.set(id, { ...item, delivery_date: item.confirmed_delivery_date || item.delivery_date, dispatch_event_id: id, dispatch_layer: 'materials' });
    });
    array(core.state.calendar.events).forEach(event => {
      ...
      rows.set(event.id, { ...(rows.get(event.id) || {}), id: event.id, job_id: event.job_id, job_number: event.job_number, supplier_name: event.title, delivery_date: event.date, delivery_time: event.time, status: event.status, dispatch_event_id: event.id, dispatch_layer: event.layer });
    });
    return [...rows.values()];
  }
  function mainBlock(event) {
    return `<button type="button" class="cal-delivery-block dispatch-calendar-event" data-source-event-id="${esc(event.dispatch_event_id)}" data-dispatch-job="${esc(event.job_id)}" data-layer="${esc(event.dispatch_layer)}"><strong>${esc(event.job_number || event.po_number || '')}</strong> ${esc(event.supplier_name)}<small>${esc(label(event.dispatch_layer))} · ${esc(label(event.status))}${event.delivery_time ? ' · ' + esc(event.delivery_time) : ''}</small></button>`;
  }
  root.DispatchOps = { mount,
    ...
    projectMain(events, deliveries, range) { return { events: controller().state.layers.staff ? events : [], deliveries: mainDeliveries(deliveries, range) }; },
    mainBlock,
