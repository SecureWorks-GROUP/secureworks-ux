...
(function (root) {
  ...
  function mount(element, options = {}) {
    ...
    function refreshEvidence() {
      ...
      refreshing = Promise.all(id ? [core.load(id).catch(() => {}), core.execution(id)] : [])
    ...
    function currentUI() {
      const id = core.state.selectedId;
      if (!uiByJob.has(id)) uiByJob.set(id, { draftId: null, draftReview: null, mailSearch: { scope: 'job', search: '' } });
    ...
    function render() {
      ...
      const focus = element.contains(focused) && (focused.name || focused.dataset.filter || focused.dataset.move || focused.dataset.layer) ? { name: focused.name, form: focused.form?.dataset.form, data: { ...focused.dataset }, type: focused.type, value: focused.value, jobId: contextJob, draftId: currentUI().draftId, start: focused.selectionStart, end: focused.selectionEnd } : null;
