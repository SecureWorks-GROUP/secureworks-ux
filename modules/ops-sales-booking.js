(function (global) {
  ...
  var state = {
    ...
    conversation: { contactId: null, caseId: null, loading: false, error: null, messages: [], generation: 0, coverage: null, conversationId: null },
    drafts: {},
    archives: {},
    sendAttempted: false,
    lastSendCall: null,
  ...
  function acceptedSlotStillCurrent(c) {
    ...
    var boundResource = c.accepted_resource_id || c.resource_id || null;
    if (boundResource && boundResource !== state.resourceId) return false;
    if (c.accepted_contact_id && c.contact_id && c.accepted_contact_id !== c.contact_id) return false;
    return true;
  }

  function workflowPost(action, body) {
    var apiUrl = global.SALES_BOOKING_API_URL || (global.window && global.window.SALES_BOOKING_API_URL);
    var apiToken = global.SALES_BOOKING_API_TOKEN || (global.window && global.window.SALES_BOOKING_API_TOKEN);
    if (apiUrl && typeof global.fetch === 'function') {
      return global.fetch(apiUrl + (apiUrl.indexOf('?') >= 0 ? '&' : '?') + 'action=' + encodeURIComponent(action), {
        method: 'POST',
        headers: {
        ...
        return resp.json().then(function (data) {
          ...
          if (resp.status === 409 && !data.code) data.code = 'cas_conflict';
          return data;
        });
      });
    }
    var previewApi = global.SALES_BOOKING_PREVIEW_API || (global.window && global.window.SALES_BOOKING_PREVIEW_API);
    if (previewApi && typeof global.fetch === 'function') {
      return global.fetch(previewApi + '?action=' + encodeURIComponent(action), {
        method: 'POST',
    ...
    if (typeof global.opsPost === 'function') {
      return Promise.resolve(global.opsPost(action, body)).catch(function (e) {
        return {
          ok: false,
          code: (e && e.code) || 'cas_conflict',
          error: e && e.message,
          http_status: e && e.status
        };
      });
    }
    return null;
  ...
  function hasDraftTransport() {
    return !!(global.SALES_BOOKING_API_URL || (global.window && global.window.SALES_BOOKING_API_URL) || global.SALES_BOOKING_PREVIEW_API || (global.window && global.window.SALES_BOOKING_PREVIEW_API) || typeof global.opsPost === 'function');
  }

  function draftIsDirty(d) {
    if (!d) return false;
    return (d.text || '') !== (d.savedText || '');
  }

  function draftIsAcknowledged(d) {
    if (!d) return false;
    if (d.savePending || d.saveQueue) return false;
    if (d.conflict || d.casConflict || d.saveError) return false;
    if (Number(d.serverRevision || 0) < 1) return false;
    return !draftIsDirty(d);
  }

  function draftBlocksApproval(d) {
    if (d && d.conflict) return true;
    if (!hasDraftTransport()) return false;
    return !draftIsAcknowledged(d);
  }

  function applyDraftAck(c, d, rec, sentText, expected, gen) {
    if (!d || gen !== d.saveGeneration) return rec;
    d.savePending = false;
    if (!rec) {
      d.saveError = 'Draft save returned nothing';
      return rec;
    }
    if (rec.code === 'cas_conflict' || rec.http_status === 409) {
      d.casConflict = true;
      d.saveError = rec.error || 'Draft revision conflict';
      return rec;
    }
    if (rec.ok === false) {
      d.saveError = rec.error || 'Draft save failed';
      return rec;
    }
    if (typeof rec.revision !== 'number') {
      d.saveError = 'Draft save returned no revision';
      return rec;
    }
    if (Number(expected) !== Number(d.serverRevision || 0)) return rec;
    if (Number(rec.revision) <= Number(d.serverRevision || 0)) return rec;
    d.serverRevision = rec.revision;
    d.revision = rec.revision;
    d.savedText = sentText;
    d.casConflict = false;
    d.saveError = null;
    if (c) {
      c.draft = {
  ...
  function flushDraftSave(c, d) {
    if (!c || !d) return Promise.resolve(null);
    d.saveQueue = false;
    var text = d.text;
    var expected = Number(d.serverRevision || 0);
    var gen = (d.saveGeneration = (d.saveGeneration || 0) + 1);
    d.savePending = true;
    d.saveError = null;
    var posted = workflowPost('sales_booking_draft', {
      case_id: c.id,
      text: text,
      human_edited: !!d.humanEdited,
      sender: d.sender || null,
    ...
    var chain = Promise.resolve(posted).then(function (rec) {
      applyDraftAck(c, d, rec, text, expected, gen);
      if (gen !== d.saveGeneration) return rec;
      if (d.saveQueue || draftIsDirty(d)) {
        if (d.casConflict || d.saveError) return rec;
        d.saveChain = flushDraftSave(c, d);
        return rec;
      }
      return rec;
    }, function (err) {
      if (gen !== d.saveGeneration) return { ok: false };
      d.savePending = false;
      d.saveError = (err && err.message) || 'Draft save failed';
      return { ok: false, error: d.saveError };
    });
    d.saveChain = chain;
    return chain;
  }

  function persistDraftToServer(c, d) {
    if (!c || !d) return Promise.resolve(null);
    if (!hasDraftTransport()) return Promise.resolve({ ok: false, reason: 'no_transport' });
    if (d.savePending && d.saveChain) {
      d.saveQueue = true;
      return d.saveChain;
    }
    if (!draftIsDirty(d) && Number(d.serverRevision || 0) > 0 && !d.casConflict && !d.saveError) {
      return Promise.resolve({ ok: true, revision: d.serverRevision, skipped: true });
    }
    return flushDraftSave(c, d);
  }

  function enqueueReassess(c, eventKey) {
    if (!c) return;
    workflowPost('sales_booking_on_event', { event_key: eventKey, type: 'inbound', case_id: c.id });
  }

  function inboundFrontier(m) {
    if (!m) return null;
    return String(m.id || '') + ':' + String(m.timestamp || '');
  }

  function coverageEnvelope() {
    var cov = (state.data && state.data.coverage) || {};
    return {
      calendar_retrieved_at: cov.calendar_retrieved_at || (state.data && state.data.retrieved_at) || null,
      leave_retrieved_at: cov.leave_retrieved_at || null,
      travel_retrieved_at: cov.travel_retrieved_at || null,
      leave_intervals: cov.leave_intervals,
      travel_minutes: cov.travel_minutes,
      leave_roster_complete: cov.leave_roster_complete === true,
      now: cov.as_of || (state.data && state.data.retrieved_at) || null
    };
  }

  function applyAssessmentToCase(c, rec) {
    var result = rec && rec.payload ? rec.payload : rec;
    if (!c || !result || typeof result !== 'object') return false;
    if (!result.status && result.exact_acceptance == null) return false;
    if (result.stale || result.actionable === false || result.classification === 'unassessed_conversation') {
      c.status = 'needs_decision';
      c.proposal_stale = true;
      c.assessment_stale = true;
      bindDraft(c);
      return true;
    }
    bindDraft(c);
    var previousProposal = c.proposal;
    if (result.status) c.status = result.status;
    if (result.reason) c.reason = result.reason;
    c.exact_acceptance = !!result.exact_acceptance;
    if (result.exact_acceptance && result.accepted_offer) {
      c.accepted_offer_id = result.accepted_offer.offer_id;
      c.accepted_start_iso = result.accepted_offer.start_iso;
      c.accepted_end_iso = result.accepted_offer.end_iso || c.accepted_end_iso || null;
      c.accepted_message_id = result.accepted_offer.message_id || c.accepted_message_id || null;
      c.accepted_resource_id = state.resourceId;
      c.accepted_contact_id = c.contact_id || null;
    }
    if (result.proposal) {
      c.proposal = result.proposal;
      c.proposal_stale = false;
      if (!previousProposal || previousProposal.start_iso !== c.proposal.start_iso || previousProposal.end_iso !== c.proposal.end_iso) {
        synchronizeProposalDraft(c, result.draft || c.proposal.draft || suggestedDraft(c));
      }
  ...
  function applyLatestInboundToCase(msgs, c) {
    ...
    if (!last) return;
    var frontier = inboundFrontier(last);
    var key = 'ghl:' + c.id + ':' + (last.id || last.timestamp || 'latest');
    if (c.exact_acceptance && c.accepted_message_id && last.id && String(c.accepted_message_id) === String(last.id)) {
      c.conversation_frontier = frontier;
      return;
    }
    if (c.conversation_frontier && c.conversation_frontier === frontier) return;
    c.conversation_frontier = frontier;
    if (c.exact_acceptance) {
      c.exact_acceptance = false;
      c.status = 'needs_decision';
    } else if (c.status === 'ready' || c.status === 'proposal' || c.status === 'waiting' || c.status === 'offer') {
      c.status = 'needs_decision';
      c.proposal_stale = true;
    }
    ...
    var posted = workflowPost('sales_booking_assess', {
      case_id: c.id,
      ...
      input: {
        ...
        suburb: c.suburb,
        now: env.now,
        calendar_retrieved_at: env.calendar_retrieved_at,
        leave_retrieved_at: env.leave_retrieved_at,
        travel_retrieved_at: env.travel_retrieved_at,
        leave_intervals: env.leave_intervals,
        travel_minutes: env.travel_minutes,
        coverage: {
          leave_roster_complete: env.leave_roster_complete === true
        }
      }
    });
    if (!posted || typeof posted.then !== 'function') return posted;
    return posted.then(function (rec) {
      if (selectedCase() !== c || state.resourceId !== resourceId || state.request !== request || c.contact_id !== contactId || c.conversation_frontier !== frontier) return;
      if (!applyAssessmentToCase(c, rec)) c.status = 'needs_decision';
    });
  }

  function evidencePending(c) {
    if (!c) return true;
    if (c.proposal_stale) return true;
    if (c.contact_id && state.conversation.contactId === c.contact_id) {
      if (state.conversation.loading) return true;
      if (state.conversation.error) return true;
    }
    return false;
  }

  function bindingMismatch(c) {
    if (!c) return false;
    var boundResource = c.accepted_resource_id || c.resource_id || null;
    if (boundResource && boundResource !== state.resourceId) return true;
    if (c.accepted_contact_id && c.contact_id && c.accepted_contact_id !== c.contact_id) return true;
    return false;
  }
  ...
  function actionKind(c) {
    ...
    if (c.assessment_stale) return 'none';
    var assessment = c.assessment && c.assessment.payload ? c.assessment.payload : c.assessment;
    if (assessment && (assessment.stale || assessment.actionable === false || assessment.classification === 'unassessed_conversation')) return 'none';
    if (c.proposal && c.proposal.kind === 'tentative') return 'none';
    if (assessment && assessment.source_version && c.source_version && assessment.source_version !== c.source_version) return 'none';
    if (evidencePending(c) || bindingMismatch(c)) return 'none';
    if (draftBlocksApproval(draftFor(c))) return 'none';
    if (acceptedSlotStillCurrent(c)) return 'confirm_booking';
    if (c.status === 'ready' || c.status === 'proposal') return 'approve_offer';
    return 'none';
  }
  ...
  function reviseProposedTime(startIso) {
    ...
    var d = synchronizeProposalDraft(c, suggested);
    return { ok: true, conflict: !!d.conflict, revision: d.revision, serverRevision: d.serverRevision, text: d.text, suggested: suggested, exact_acceptance: !!c.exact_acceptance, end_iso: c.proposal.end_iso };
  }

  function synchronizeProposalDraft(c, suggested) {
    var d = draftFor(c);
    if (d.humanEdited && d.text !== suggested) {
      d.conflict = true;
    ...
    if (!d.conflict) persistDraftToServer(c, d);
    return d;
  }
  ...
  function reviseProposedSlot(startIso, endIso) {
    ...
    var d = synchronizeProposalDraft(c, suggestedDraft(c));
    return { ok: true, exact_acceptance: !!c.exact_acceptance, start_iso: startIso, end_iso: endIso, conflict: !!d.conflict };
  }
  ...
  async function restoreCase(id) {
    ...
    if (c && c.restorePending) return { ok: false, reason: 'restore_pending' };
    if (c) { c.restorePending = true; c.restoreError = null; }
    try {
      var response = await workflowPost('sales_booking_restore', { case_id: id });
      if (!response || response.ok !== true || response.restored === false || response.http_status >= 400) {
        throw new Error(response && (response.error || response.reason) || 'Restore was not acknowledged.');
      }
      if (state.archives[id]) state.archives[id].restored = true;
      cases().forEach(function (row) {
        if (row.id === id && row.archived) row.archived.restored = true;
      });
      return { ok: true, contact_id: (c && c.contact_id) || (rec && rec.contact_id), crm_deleted: false };
    } catch (error) {
      if (c) c.restoreError = error.message || 'Restore failed.';
      return { ok: false, reason: 'restore_failed', error: error.message, crm_deleted: false };
    } finally {
      if (c) c.restorePending = false;
    }
  }
  ...
  function draftFor(c) {
    ...
    if (!key) return { text: '', revision: 0, serverRevision: 0, humanEdited: false, sender: null };
    if (!state.drafts[key]) {
      state.drafts[key] = { text: '', revision: 0, serverRevision: 0, humanEdited: false, sender: null, case_id: key };
    }
    if (state.drafts[key].serverRevision == null) state.drafts[key].serverRevision = 0;
  ...
  function coverageGaps(data) {
    ...
    if (cov.leave_roster_complete === false) {
      gaps.push('Staff availability was read; scoper roster completeness is not declared, so missing rows are not proof of no leave.');
    }
  ...
  function calendarItems() {
    var items = events().map(function (ev) {
      return Object.assign({}, ev, { id: ev.case_id || ev.event_id, layer: ev.layer || 'confirmed' });
    });
    cases().forEach(function (c) {
      ...
      if (!heldOffer && (c.proposal_stale || (c.status !== 'proposal' && c.status !== 'ready'))) return;
      items.push({
        id: c.id,
        ...
        suburb: c.suburb,
        layer: heldOffer ? 'offer' : 'proposal',
        bookingCase: c
      });
    });
    return items.filter(function (item) {
      return dayIndexFromIso(item.start_iso, state.weekStart) != null && hourFromIso(item.start_iso) != null;
    });
  }
  ...
  function renderCalendar() {
    ...
    for (var d = 0; d < 5; d++) {
      ...
      items.forEach(function (item) {
        if (dayIndexFromIso(item.start_iso, state.weekStart) !== d) return;
        if (item.bookingCase) body += renderWindows(item.bookingCase);
        body += renderEvent(item, item.layer);
      });
    ...
    return '<div class="layers">' +
      ...
      '<div class="dayselect" aria-hidden="true">' + DAYS.map(function (name, i) {
        return '<span class="small muted">' + name.slice(0, 3) + ' <b>' + addDays(state.weekStart, i).slice(8, 10) + '</b></span>';
      }).join(' ') + '</div>' +
  ...
  function renderAgenda() {
    var items = calendarItems().filter(function (item) { return state.layers[item.layer]; });
    ...
    return items.map(function (it) {
      return '<button type="button" class="lead" data-booking-case="' + esc(it.id) + '"><strong>' + esc(String(it.start_iso).slice(0, 10) + ' ' + String(it.start_iso).slice(11, 16)) + '</strong> ' + esc(it.display_name || it.subject || '') + ' · ' + esc(it.suburb || '') + '</button>';
    }).join('');
  }
  ...
  function renderMessages() {
    ...
    var cover = conv.coverage || {};
    var coverNote = cover.has_more
      ? '<div class="comms-empty">Older messages exist. This page is not a complete thread. Source ' + esc(cover.source || 'provider') + ' at ' + esc(cover.retrieved_at || '') + '.</div>'
      : (cover.retrieved_at ? '<div class="comms-msg-meta">Latest read ' + esc(cover.retrieved_at) + (cover.conversation_id ? ' · conversation ' + esc(cover.conversation_id) : '') + '</div>' : '');
    return coverNote + conv.messages.map(function (msg) {
      var dir = msg.direction === 'outbound' ? 'outbound' : 'inbound';
      var ts = msg.timestamp ? String(msg.timestamp) : '';
      var type = String(msg.type || 'SMS');
      var mid = msg.id ? ' · id ' + esc(msg.id) : '';
      return '<div class="comms-msg ' + dir + '"><div>' + esc(msg.body || msg.subject || type) + '</div><div class="comms-msg-meta">' + esc(ts) + (msg.sender_name ? ' · ' + esc(msg.sender_name) : '') + ' · ' + esc(type) + mid + '</div></div>';
    }).join('');
  }
  ...
  function renderDetail() {
    ...
    var holdNote = SEND_HOLD
      ...
      : '';
    var stale = !!(c.proposal_stale || evidencePending(c));
    var pick = stale
      ? 'Invalid — waiting for refreshed evidence'
      : (c.proposal && c.proposal.start_iso ? String(c.proposal.start_iso).replace('T', ' ').slice(0, 16) : '');
    var windowLabel = stale
      ? 'Previous window is not current. Do not approve until assessment catches up.'
      : (c.proposal && c.proposal.kind === 'tentative'
        ? ('Tentative only. ' + ((c.proposal.staff_next && c.proposal.staff_next[0]) || 'Confirm leave and travel before treating this as Ready.'))
        : ((c.proposal && c.proposal.window_label) || 'No customer window extracted'));
    var actionLabel = kind === 'confirm_booking' ? 'Confirm booking (held)' : kind === 'repair' ? 'Calendar repair (held)' : kind === 'approve_offer' ? 'Approve offer (held)' : 'Approval not offered';
    var timeInput = stale
      ? '<p class="small muted">Proposed time is invalid until the new inbound is assessed.</p>'
      : (c.proposal && c.proposal.start_iso
    ...
    var conflict = d.conflict
      ? '<div class="notice error">Time changed. Your edited draft was kept. Update your draft before approval.<p>' + esc(d.suggested || '') + '</p></div>'
      : '';
    ...
    return '<div class="detailhead"><span class="smalltag">' + esc(statusLabel(c.status)) + '</span><h2>' + esc(c.display_name || 'Enquiry') + '</h2><p class="muted">' + esc(c.suburb || '') + (c.contact_id ? '' : ' · no GHL contact') + '</p></div>' +
      ...
      '<div class="senderline">' + senderLine(res) + ' · To this case only · Saved revision ' + esc((d.serverRevision || 0)) + '</div>' +
      (d.savePending ? '<p class="small muted">Saving draft…</p>' : '') +
      (d.casConflict ? '<p class="notice error" role="alert">Draft conflict. Your wording is kept. Retry save before approval.</p>' : '') +
      (d.saveError && !d.casConflict ? '<p class="notice error" role="alert">Draft save failed. Your wording is kept.</p>' : '') +
      (draftIsDirty(d) && !d.savePending && !d.casConflict && !d.saveError ? '<p class="small muted">Unsaved draft. Approval waits for the acknowledged revision.</p>' : '') +
      '<label class="small muted">Draft SMS</label>' +
      '<textarea data-booking-draft="1" aria-label="Draft SMS">' + esc(d.text || '') + '</textarea>' +
      '<button type="button" class="primary" data-booking-approve="1"' + (SEND_HOLD || kind === 'none' || stale || draftBlocksApproval(d) || !resolveSender(res).resolved ? ' disabled' : '') + '>' + esc(actionLabel) + '</button>' +
      archiveBlock +
  ...
  function renderHTML() {
    ...
    return '<div class="page"><div class="pagehead"><div><h1>Build the week</h1><p>Customer windows, the diary, and the next conversation.</p></div>' +
      ...
      '</select></label><button type="button" data-booking-refresh="1"' + (state.loading ? ' disabled' : '') + '>' + (state.loading ? 'Refreshing…' : 'Refresh') + '</button></div></div>' + notice +
  ...
  function applySearch(value) {
    state.search = value;
    var host = root();
    var queue = host && host.querySelector('.queuelist');
    var count = host && host.querySelector('.queue .count');
    if (queue) queue.innerHTML = renderQueue();
    if (count) count.textContent = String(visibleCases().length);
    if (!queue && !count) render();
  }

  function clearConversation() {
    state.conversation = { contactId: null, caseId: null, loading: false, error: null, messages: [], generation: state.conversation.generation + 1, coverage: null, conversationId: null };
  ...
  async function bookingRead(params) {
    ...
    if (apiUrl && typeof global.fetch === 'function') {
      ...
      var url = apiUrl + join + 'action=sales_booking_read'
    ...
    if (preview) {
      var previewUrl = preview + '?resource=' + encodeURIComponent(params.resource) + '&week_start=' + encodeURIComponent(params.week_start);
      var previewResp = await global.fetch(previewUrl, { cache: 'no-store' });
      if (!previewResp.ok) throw new Error('Preview calendar read failed (' + previewResp.status + ')');
  ...
  async function load(resourceId, weekStart, opts) {
    ...
    try {
      ...
      (data.cases || []).forEach(function (row) { bindDraft(row); });
    } catch (e) {
      if (request !== state.request) return;
  ...
  async function loadConversation(contactId, caseId) {
    ...
    render();
    try {
      var previewConvo = global.SALES_BOOKING_PREVIEW_CONVERSATION_URL || (global.window && global.window.SALES_BOOKING_PREVIEW_CONVERSATION_URL);
      var resp;
      if (previewConvo) {
        resp = await global.fetch(previewConvo + '?contactId=' + encodeURIComponent(contactId), {
          cache: 'no-store',
        ...
        resp = await global.fetch(base + '?action=get_conversation&contactId=' + encodeURIComponent(contactId), {
          headers: headers,
          signal: convoAbort && convoAbort.signal
        });
      }
      ...
      if (data.fixture) throw new Error('Fixture conversation is refused. Provider read required.');
      var msgs = data.messages || [];
      msgs.sort(function (a, b) { return String(a.timestamp || '') < String(b.timestamp || '') ? -1 : 1; });
      state.conversation.messages = msgs;
      state.conversation.coverage = data.coverage || null;
      state.conversation.conversationId = data.conversation_id || data.conversationId || null;
      state.conversation.loading = false;
      var selected = selectedCase();
      if (selected && selected.contact_id === contactId) {
        var applied = applyLatestInboundToCase(msgs, selected);
  ...
  function attemptApprove() {
    var c = selectedCase();
    var kind = actionKind(c);
    var d = c && draftKey(c) ? draftFor(c) : { text: '', revision: 0, serverRevision: 0 };
    var route = resolveSender();
    if (kind !== 'approve_offer' && kind !== 'confirm_booking' && kind !== 'repair') {
      var blocked = evidencePending(c) ? 'evidence_pending' : (bindingMismatch(c) ? 'binding_mismatch' : (draftBlocksApproval(d) ? 'draft_unacknowledged' : 'not_actionable'));
      state.lastSendCall = { action: 'none', reason: blocked, case_id: c && c.id, persisted: false };
      return { ok: false, held: true, sent: false, booked: false, waiting: false, reason: blocked, action: 'none', persisted: false };
    }
    if (c && c.contact_id && state.conversation.contactId === c.contact_id) {
      if (state.conversation.loading) {
        return { ok: false, held: true, sent: false, booked: false, waiting: false, reason: 'conversation_pending', action: 'none', persisted: false };
      }
      if (state.conversation.error) {
        return { ok: false, held: true, sent: false, booked: false, waiting: false, reason: 'conversation_error', action: 'none', persisted: false };
      }
    }
    var payload = {
      case_id: c && c.id,
      kind: kind,
      offer_id: c && c.proposal && c.proposal.offer_id || undefined,
      draft_revision: Number(d.serverRevision || 0),
      start_iso: c && c.proposal && c.proposal.start_iso,
      end_iso: c && c.proposal && c.proposal.end_iso,
      resource_id: state.resourceId,
      recipient: c && c.contact_id,
      sender: route.number,
      source_version: c && c.source_version || null,
      text: d.text || '',
      persisted: false
    };
    state.lastSendCall = payload;
    if (kind === 'approve_offer' || kind === 'confirm_booking') {
      var save = persistDraftToServer(c, d);
      var sendApprove = function () {
        if (draftBlocksApproval(d)) {
          state.lastSendCall = { action: 'none', reason: 'draft_unacknowledged', case_id: c.id, persisted: false };
          return { ok: false, reason: 'draft_unacknowledged', action: 'none', persisted: false };
        }
        payload.draft_revision = Number(d.serverRevision || 0);
        payload.text = d.savedText || d.text || payload.text;
        state.lastSendCall = payload;
        return workflowPost(kind === 'confirm_booking' ? 'sales_booking_confirm' : 'sales_booking_approve', payload);
      };
      if (save && typeof save.then === 'function') {
        save.then(function (rec) {
          if (rec && (rec.code === 'cas_conflict' || rec.http_status === 409 || rec.ok === false)) return rec;
          return sendApprove();
        });
      } else sendApprove();
    }
    ...
    if (kind === 'confirm_booking' && !acceptedSlotStillCurrent(c)) {
  ...
  if (global.document) {
    global.document.addEventListener('click', function (e) {
      ...
      var refreshBtn = e.target.closest && e.target.closest('[data-booking-refresh]');
      if (refreshBtn) {
        e.preventDefault();
        load(state.resourceId, state.weekStart, { refresh: true });
        return;
      }
      ...
      if (rest) {
        e.preventDefault();
        restoreCase(rest.getAttribute('data-booking-restore')).then(render);
        render();
        return;
      }
    ...
    global.document.addEventListener('input', function (e) {
      ...
      if (e.target.matches && e.target.matches('[data-booking-draft]')) {
        ...
        d.conflict = false;
        d.sender = resolveSender().number;
        persistDraftToServer(c, d).then(function () {
          var box = root() && root().querySelector('[data-booking-draft]');
          if (global.document && box && global.document.activeElement === box) return;
          render();
        });
      }
    });
  ...
  var api = {
    ...
    persistDraftToServer: persistDraftToServer,
    applyLatestInboundToCase: applyLatestInboundToCase,
    draftIsAcknowledged: draftIsAcknowledged,
    bindDraft: bindDraft,
    draftKey: draftKey,
    draftFor: draftFor,
