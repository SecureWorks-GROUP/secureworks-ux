...
## Context pipeline and weekly review

Open a job's **Context** view to see the company-wide Context pipeline above the job-specific information. **Refresh status** reads database switches and effective lanes without changing them. Shared model calls use the backend-reported cap (400 in the approved contract), separately from extraction slots; unavailable budget data is not zero. The panel separates queued evidence, oldest pending source time, missing source dates, last completed pass, ready-job lower bounds, coverage and weekly human accuracy. Pending stop alerts are notification records, not proof of delivery. Business-event dates use `event_at`; missing source dates are not replaced by ingestion time.

Expand **Review weekly facts**, choose a Monday (blank selects the previous completed week), and press **Load weekly review** to read the stored sample. Loading does not draw a sample or save verdicts. The panel shows the 40-fact target alongside actual, missing and unjudged counts; missing facts do not count as correct. Compare each fact with its excerpt and source references, choose **True**, **False** or **Wrong job**, then explicitly press **Save my verdict**. Only **False** enables the invented-payment checkbox; that finding is intended to stop extraction immediately.

Saving uses the existing signed-in operator token through `opsAuthHeaders` / `opsPost`; the client submits no `judged_by`. The B4 backend must authorise and derive the actor. A successful save reads back the sample and refreshes status. Other unsaved choices survive that readback by week and fact identity only while their stored row is unchanged. If the server row changed, its latest values replace those choices with a visible notice. A failed save/readback keeps the selections and asks the operator to reload before retrying. Choices are transient: loading a week or remounting Context clears them.

The required backend actions are `context_pipeline_status`, `context_accuracy_sample` and `context_accuracy_verdict`. See the [local proof record](docs/evidence/context-pipeline-2026-09-11/README.md) for the verification scope; regression coverage lives in `tests/e2e/context-pipeline-panel.spec.js`.
