...
- Pipeline kanban cards (fencing, patio, repair) in the Scheduled, In Progress and Repairs On Site columns carry a date chip showing the job's CURRENT schedule: the next visit not yet happened, else the most recent past visit, start date only. It reads the pipeline feed's `next_scheduled_date ?? last_scheduled_date`, never `first_scheduled_date` (the historical earliest booking, ghost rows included). The "N sched" count beside it still counts every assignment row and is tracked separately. Guard: `tests/e2e/ops-card-schedule-date.spec.js`.
...
- Every calendar surface clamps an inverted assignment span (`scheduled_end` before `scheduled_date`) to a single day at its start via `CalOpsCore.spanEnd`, so a bad row never paints its bar over a lane-mate in Schedule view and never silently disappears from the Crew view, the summary conflict counter, or the make-safe crew planner. Long bar labels ellipsize inside their own bar (guarded by `tests/e2e/ops-schedule-lane-overlap.spec.js` and `tests/e2e/ops-inverted-span-surfaces.spec.js`).
- The schedule modal and assignment form serialise their computed end dates in local time (`localDateStr`), never `toISOString()` — in Perth (UTC+8) UTC serialisation lands the previous day, which wrote a 1-day assignment as an inverted span and cut every longer span one day short (guarded by the UTC+8 write-path tests in `tests/e2e/ops-inverted-span-surfaces.spec.js`).
...
- **Collision-safe writes (dragv2 ON)**: a Schedule bar can be several same-person day rows, and `job_assignments` is unique on `(job_id,user_id,scheduled_date)`, so every V2 move/resize is staged through `CalOpsCore.stageCollisionSafeMoves` against the job-scoped occupancy (`calStagingOccupancy`: loaded window + the job's full `job_detail` assignment set) before anything is written. What the user sees: a whole-bar drag either moves every row or moves nothing, with one warning toast naming the genuine existing visits that block it; a single-row move or resize onto a date that already holds a visit is skipped ("Already scheduled there — existing visit kept", or naming a cancelled/completed visit the calendar hides) and both visits are kept; two rows of one bar collapsing onto one date says so instead of blaming an existing visit. The full invariant (ghost observer rows, blocker counting, ordering) is owned by `AGENTS.md` — do not re-derive it here.
- Crew-change drags keep the G2 allocation semantics (delete + `create_assignment` with NO confirmationStatus, so the new installer gets the allocation SMS); with dragv2 ON the same staging check runs against the target crew's key first, so a colliding reassign is skipped before the delete — only the span/duration math and this pre-write check changed.
- Pure date math and the staging helpers live in the `// <calendar-ops-core>` block; guarded by `tests/e2e/cal-workdays.spec.js` (extracts and evaluates the shipped code). Collision behaviour end to end (feed vs `job_detail` occupancy, all-or-nothing bars, skipped single rows, ghost mirrors, cancelled/completed holders, toast wording) is guarded by `tests/e2e/ops-calendar-move-collision.spec.js`. Drag itself is guarded by `tests/e2e/cal-drag-real-input.spec.js` — trusted pointer input only, both views (synthetic-event checks once masked a Schedule view with no drag wiring at all), including a weekend-crossing fixture that must render as broken segments in BOTH views and reschedule the whole job when either segment is dragged, plus Perth-pinned (UTC+8) checks that a flag-off modal create writes the intended local span and that a single-crew move onto mid-span leave raises the combined warning. The same spec guards the default-on gating: a plain boot (no param, no localStorage key) drags as V2, the localStorage `'0'` kill switch still reaches the untouched V1 calendar-delta drag, and the URL flag beats localStorage in both directions.
