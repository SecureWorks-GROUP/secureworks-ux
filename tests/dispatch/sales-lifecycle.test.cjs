...
test('Performance measure clicks expose missing evidence and restore focus on close', async () => {
  ...
  assert.match(dialog.textContent, /Report unavailable|No report is published/);
  assert.match(dialog.textContent, /Named evidence is unavailable|evidence are unavailable/);
...
for (const quotes of [undefined, []]) {
  test(`C1 exposes related follow-up when quotes are ${quotes ? 'empty' : 'missing'}, without measuring sends`, async () => {
    ...
    assert.equal(h.elements('article').length, 0);
    assert.doesNotMatch(dialog.textContent, /Follow-up customer/);
    assert.match(dialog.textContent, /Named evidence is unavailable|No document-evidenced send queue|follow-up stage queue/);
    assert.equal(h.context.SalesPerformance.adapt(h.context.SalesPerformance.state.data.rows[0]).measures.C1.value, null);
    h.context.SalesPerformanceDetail.close();
    assert.equal(h.elements('dialog').length, 0);
