...
test('calendar layers keep event identity and link back to the original job',async({page})=>{
  await setup(page);await page.locator('input[data-layer="materials"]').uncheck();await expect(page.locator('[data-id="po:source-b"]')).toHaveCount(0);await page.locator('input[data-layer="materials"]').check();await page.locator('[data-id="po:source-b"]').click();await expect(page.getByRole('heading',{name:/FIX-102/})).toBeVisible();await page.getByRole('button',{name:'Patios',exact:true}).click();await expect(page.locator('.dp-job')).toHaveCount(1);expect(await page.evaluate(()=>core.state.jobs.length)).toBe(2);
});
...
test('actual source fields and HTML-only mail render safely without invented links',async({page})=>{await setup(page);await page.evaluate(async()=>{fixture.records.a.job.scope_json={job:{runs:[{length:12.5}]}};fixture.records.a.job.pricing_json={customer_quote:{total:123}};fixture.records.a.media=[{id:'photo',name:'Captured site photo',storage_url:'https://example.test/photo.jpg'},{id:'missing',name:'Missing original'},{id:'script',name:'Unsafe source',storage_url:'javascript:alert(1)'}];fixture.records.a.communications=[{id:'html-mail',job_id:'a',subject:'HTML-only captured mail',body_html:'<p>First paragraph</p><script>window.injected=true</script><p>Second paragraph</p>'}];await core.load('a');});await expect(page.getByText('Stored job scope',{exact:true})).toBeVisible();await expect(page.getByRole('link',{name:'Open original',exact:true})).toHaveCount(1);await expect(page.getByRole('link',{name:'Open original',exact:true})).toHaveAttribute('href','https://example.test/photo.jpg');await page.getByRole('tab',{name:'Email',exact:true}).click();await page.getByRole('button',{name:/HTML-only captured mail/}).click();await expect(page.getByText(/First paragraph/)).toBeVisible();expect(await page.evaluate(()=>window.injected)).toBeUndefined();});
