...
test('uses cash payable (not old full-super net) across recent activity, history, profile, and detail', async ({ appPage: page }) => {
  ...
  await expect(recentComplete).toContainText('$376.00');
  await expect(recentComplete).not.toContainText('$400.00');
  await expect(recentComplete).not.toContainText('$352.00');
  ...
  await expect(historyComplete).toContainText('$376.00');
  await expect(historyComplete).not.toContainText('$352.00');
  ...
  await expect(profileList).toContainText('$376.00');
  await expect(profileList).not.toContainText('$352.00');
