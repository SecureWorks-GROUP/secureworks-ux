...
test('Henry Financial invoices jobs with WO-trade deducts and a 12% super preview', async ({ appPage: page, feedRequests }) => {
  ...
  await expect(money).toContainText('Your share of super (6%)−$3.00');
  await expect(money).toContainText('Company covers the other 6%$3.00');
  await expect(money).toContainText('You get paid$47.00');
