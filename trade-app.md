...
- The fencing Board defaults to the current Perth Monday-Sunday week. Previous, next, This week, and Unscheduled controls filter the complete authorized response in memory without a navigation or refetch. Unscheduled is the single home for every null-date row and shows each one under its own status column (not just Ready), so no authorized card is unreachable; the unmapped-status banner is always counted board-wide, never from the filtered week. On phone widths the six status buttons are direct navigation for a native horizontal, snap-scrolling column pager; week controls are separate buttons and never share the swipe gesture. That pager (markup, CSS and wiring) is scoped to `.tjb-wrap.fencing` — the make-safe Board keeps its stacked columns and plain status counters at every width.
...
- `scripts/test-fencing-manager-visibility.js` guards the managed-vertical lead contract against the extracted `// <trade-visibility-core>`, `// <trade-calendar-source>` and `// <fencing-board-core>` blocks: the Everyone lens is granted by `managed_verticals` (not by role) and denied to an ordinary installer, caches split by identity / vertical / lens, the fencing board dedupes stale and duplicate open rows, excludes an explicitly foreign tenant row, filters Perth weeks while every undated card stays reachable under Unscheduled in its own status column, and keeps unknown statuses visible in Attention with a board-wide job count. It also guards `// <trade-workorder-auth>` (an untyped work order is never dropped, another tenant/vertical is, the job-detail entry point authorises its own matched order, and a widened viewer with no `org_id` fails closed while an ordinary own-only response still renders), asserts the phone column pager is scoped to `.tjb-wrap.fencing`, and proves `trade-calendar.v1` rejects a wrong schema, vertical or missing `truncated`. It runs in PR CI. The end-to-end counterpart is `tests/e2e/fencing-manager-visibility.spec.js` (see `README-tests.md`).
