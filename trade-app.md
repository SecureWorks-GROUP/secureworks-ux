...
- Because a line pays a person, the payload builder cleans and validates: names are trimmed/whitespace-collapsed (`_woCleanLabourLines` — production had "Tendo  " ≠ "Tendo"), untouched "+ Add labour line" template rows are dropped, a line carrying money must name someone, and a named line must carry hours > 0 and rate > 0. Name→user matching itself is server-side and deterministic (unique full-name/first-name/prefix match); anything unmatched lands in the office's query_note, never guessed.
