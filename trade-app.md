...
- Repair cards (`.jc.rp`, teal `--jt-rp`) add the builder's instruction numbers after our job number via `tradeRepairRefs`: `WO <work order>`, then the PO, then the claim ref only when it differs from the work order; the party row reads Builder from `builder_company_name`. Absent fields are not drawn. Guard: `tests/e2e/trade-repair-vertical.spec.js`; vertical rule in `AGENTS.md` → "Repair vertical".
...
- **Typed query** = authenticated database search for every trade, but only while
  the **All** tab is active. Card chips, tap/open rules, own-card dedupe and
  missing place/number titles are owned by `AGENTS.md` → "ALL-TAB SEARCH CARDS".
...
- Guarded by `tests/e2e/all-jobs-feed.spec.js` (feed paging, server-lens
  authority, installer unchanged) and `tests/e2e/trade-all-search-truth.spec.js`
  (search-card truth; contract in `AGENTS.md`).
...
- **Scope / quote writing (TRD-5)**: allocated trades see quote numbers, job writing, compact WO `scope_items`, and **every** job video (walkthrough + others) on the Scope tab, Files, Photos, and the make-safe report header. `$` / rates / totals stay off the new quote-pack and compact WO surfaces unless `tradeCanSeeSowPricing()`; who that is, and Cost Breakdown / Crew Charges money, are owned by `AGENTS.md` → "MONEY IS OFFICE-ONLY". Captain lock: the full priced MakeSafe / supplier work-order PDF is never linked, embedded, listed, shown as a photo, offered as an external/source link, or played as a video for installer/trade tiers (Open full WO / iframe / both Files document lists / galleries / MakeSafe links) — office keeps the PDF. Filename/type/URL aliases (`Site-WO.pdf`, `builder_pack`, a photo row whose URL is the WO PDF) use the same `isPricedWorkOrderDocument()` / `filterTradeGalleryMedia()` / `filterTradeSowLinks()` doors. `normaliseTradeExternalLinks()` keeps kind/type/file/PDF metadata so a typed work-order or supplier-WO signed URL with a generic “Open link” label is still office-only. `isPricedWorkOrderDocument()` / `isJobVideo()` check every typed field (`kind` / `document_type` / `role`) before video heuristics, so `type: video` cannot smuggle a WO PDF into a player. After that typed check, an explicit video type or `video/*` MIME plays even when the label says “work order” and the signed URL has no extension; fuzzy WO-name matching applies only to PDF/document bytes. MakeSafe builder-email URLs go through the same office-only WO href filter, using nearby sentence context and fail-closed opaque signed document URLs. Trade-visible note bodies (bottom Notes, Log, pending, optimistic, Quick Notes sheet) go through `escSowText()`. Free-text redaction covers cue-word totals, `8.8k`, `/day` rates, WO/client/MakeSafe prose, and video labels. Job media with `visible_to_trades: false` is hidden; untagged allocated job videos still play. A `scope_json` walkthrough flag with no playable URL is an honest “Walkthrough recorded. File not available.” — the section is not omitted. Prefers TRD-4 enriched `trade_job_detail` media URLs when present; otherwise plays `data.media`. Guard: `scripts/test-trade-scope-media.js` + `tests/e2e/trade-scope-videos-pricing.spec.js`.
...
- **Henry's weekly work-order invoice:** Financial opens the same hub as every other trade. **Weekly Invoice** is the job-centric `generate_trade_invoice` path (`submitJobCentricInvoice` with `gst_on`) — `invoice_type: per_metre` never dead-ends on an empty work-order week, and an email `/henry|emeka/` heuristic is not used. Week selection for Weekly Invoice is owned by `AGENTS.md` → "Trade weekly invoice: one per week". **+ Add job** on a Mon–Sun day then chooses which day a non-prefilled job sits on. `my_hours` assignments with a null `scheduled_date` do not prefill a card — undated work is added onto a chosen day. Scheduled / confirmed / in_progress work is invoiceable on this job path; Complete is the job-detail wizard, not a Pay-tab gate. Typed `search_all_jobs` (≥2 chars) adds a card with `assignment_id: null` and `scheduled_date` = the chosen day, submitted as `extra_items` (`manually_added`). Per-metre cards default to **Hours**; a real work-order id unlocks **Work Order** mode so a hydrated server money block (allocated amount minus named **Paid to other trades** lines) can stay on the card — Henry is not forced onto typed-dollar WO mode for those. Net on a WO card is what SecureWorks owes him; the footer shows that net, then the same 2026-09-18 super split as My money (preview when the hours payload omitted `super_rate`; submit still never sends super figures). This week's invoiceable `my_work_orders?mode=all` rows prefill those cards — hydrate authorizes only rows that are not already invoiced, `can_invoice` or `can_add_to_weekly_invoice`, and not dated outside the selected week (undated rows still pass pending a Firstmate ruling). `negative_charges` land as pass-through deducts merged by `source_line_id` so a local labour edit cannot drop a server deduction; hydrate overwrites `wo_allocated` from the current `subtotal`, drops `source_line_id` pass-throughs the WO no longer owns, and replaces a same-ID line's amount/name with current server truth, so a restored or edited draft cannot submit last week's money. An offline job-centric replay queues the same `final_deductions` as the online payload, stamped with the authenticated account; replay drops invoice actions that do not belong to the current user, aborts if invoice auth generation changes between check and send, revalidates work-order authorization for `submit_work_order_invoice`, and logout clears them. Direct invoice writes (`generate_trade_invoice`, `submit_trade_invoice`, `save_trade_invoice_draft`, `delete_trade_invoice`, `submit_work_order_invoice`, `attach_invoice_pdf`) pass a context-bound `beforeSend` so an account switch during `getAuthToken()` cannot send — response guards stay as a second fence. Online timeouts and Failed to fetch on those writes use the same reconcile-or-persist path as the offline queue (including weekly WO save/submit), and those network failures stay `ambiguous` even when `navigator.onLine` is already false so replay reconciles before resend. Direct Save Draft is single-flight so concurrent taps cannot mint parallel drafts. Every invoice submit/generate path (`generate_trade_invoice`, `submit_trade_invoice`, `submit_work_order_invoice`) shares the same process-wide in-flight lock so concurrent confirmations cannot issue parallel online POSTs. That lock and the offline queue mutations are cross-tab safe: Web Locks are preferred for financial writes and replay; otherwise a compare-and-swap claim is renewed for the whole in-flight request. Queue mutations wait for the lock and never apply unlocked. Offline replay drops an item only on true invoice success, treats nested job identities as exact targets so uncommitted job-centric writes can retry, and re-reads the invoice listing before resending an ambiguous item — it does not invent a new backend idempotency contract. The pending-write fingerprint includes `week_start`/`week_ending` so distinct weeks never suppress each other. An exact draft/invoice/work-order identity is treated as landed for ambiguous replay in any status, including draft and `pending_ops_review`. `authorizeWorkOrders` replaces the authorized set from the latest authenticated `my_work_orders` result (ids no longer present drop); offline WO replay always re-reads that feed and fails closed rather than trusting a cached id. The pending-write fingerprint includes `week_start`/`week_ending` so distinct weeks never suppress each other. An exact draft/invoice/work-order identity is treated as landed for ambiguous replay in any status, including draft and `pending_ops_review`. `authorizeWorkOrders` replaces the authorized set from the latest authenticated `my_work_orders` result (ids no longer present drop); offline WO replay always re-reads that feed and fails closed rather than trusting a cached id. Queue sync is single-flight; persist merges remaining items with the latest stored queue by stable item id so an enqueue during replay is not overwritten. Financial queue items carry a local `client_request_id`; a timeout marks the item ambiguous and the next pass reconciles only on an exact draft/invoice/work-order identity or an equivalent payload fingerprint — a week-only match is left unresolved and does not suppress a distinct same-week intent. Saving an existing draft sends `draft_id`. Stripping an unauthorized `work_order_id` also clears that server-owned money. A match on an existing assignment card still re-renders. A failed hydrate, weekly WO load, or hub read shows Retry. Hydrate pending still waits before submit so a complete WO's server block can land; a failed hydrate does **not** lock Hours, searched jobs, or invoice-level deducts — it only blocks when an included card is already in WO mode. Unmatched per-metre cards stay Hours until they have a real `work_order_id`. Hydrate binds an unbound assignment card only when the work-order completion/schedule date matches that card's day — a later visit of the same job is a separate card. If Henry already typed Hours (or locked Hours) while hydrate was pending — including an in-focus Hours field that DOM sync reads before `onchange` — the card keeps Hours; the WO id still attaches so he can switch explicitly. Job-centric `_jobCards` persist in a user-scoped `sw_inv_draft_<user>` session draft (with `is_per_metre` and invoice-level `invLumpLines`) and restore on Financial reload for that same authenticated user; logout clears in-memory cards and that user's draft. Invoice API responses and late write callbacks (`my_trade_invoices`, `get_trade_invoice`, `my_hours`, `my_work_orders`, `generate_trade_invoice`, `save_trade_invoice_draft`, `submit_trade_invoice`, `delete_trade_invoice`) are dropped when the authenticated user or request generation no longer matches. Restore still fetches `my_hours` so the WO hydrate and submit gate arm once `invoice_type` is known. A restored `work_order_id` is reconciled against the current hydrate's authorized set before it can stay on a card or submit — a stale id is stripped and the card returns to Hours. No-ID pass-throughs merge as a multiset so two same-name same-amount deducts are not collapsed. Identity is the work-order id: a second WO on the same job/day gets its own card. **+ Add amount** sits on the same card as hours / labour-line actions — a normal option for every job-centric trade, not a Henry mode and not a separate product heading. Invoice-level **+ Add amount** (`#btnAddInvLump`) is the same shape without a job (`final_deductions` + negative `extra_items`). Hours-card lumps ride both `extra_items` (`source: 'invoice_final_deduction'`) and `final_deductions`; WO-card lumps stay on the work-order extra as `wo_lump_lines` so they are not double-counted. **+ Add job** still invoices any other job on the same builder. The work-order weekly builder remains an extra **Work Order Invoice** door and still calls the published backend contract in `secureworks-backend/docs/trade-weekly-work-order-invoice-contract.md`. That extra door loads `my_work_orders?mode=all&type=fencing&status=complete&page_size=500`, selects work orders by their Perth business date, and sends only work-order IDs, acknowledged crew-charge line IDs, direct-labour user IDs/hours, and explicit final payout deductions. A work-order hub or Cost Breakdown entry that `can_add_to_weekly_invoice` opens that order's Perth week and preselects it. An empty week offers **Invoice jobs**, which is the same job-centric builder. `save_trade_invoice_draft` must return complete matching job blocks plus finite server-owned totals before Submit unlocks; stale save responses cannot replace a newer week or edit revision. `generate_trade_invoice` revalidates the same source selections and creates the Xero draft. The browser never submits scope prices, crew amounts, direct-labour rates, job subtotals, grand total, or TO BE PAID. A `weekly_work_order` draft/detail is rendered by `source_work_order_id`, preserving separate work orders on the same job. `XERO_PUSH_FAILED` means the local invoice exists and must not be submitted again.
- **`my_work_orders` tenant/vertical fields:** the Work Order hub reads `my_work_orders?mode=all` (same managed-vertical widen as the weekly builder) so unassigned managed WOs appear. Job-detail Cost Breakdown uses `mode=all` for office and `mode=mine` for everyone else (`AGENTS.md` → "MONEY IS OFFICE-ONLY"), then filters that list through `workOrdersForViewer` (tenant + managed-vertical lens) and matches only the viewed `work_order_id` — it never attaches another WO on the same job. A same-tenant patio WO is not invoiceable from a fencing lead's job detail. Authorization for `invoiceWorkOrder` is that authenticated, lens-filtered response itself — every surface that can reach `submit_work_order_invoice` registers the order it renders from its own read, so a hub visit is never a precondition for the ordinary job-detail invoice. `invoiceWorkOrder` always sends `gst_on` (`GST_CHOICE_REQUIRED` is a missing-choice 422, not a rate problem), and late `submit_work_order_invoice` confirm/response/offline-queue callbacks are dropped when the invoice API context no longer matches. On top of that server truth the client narrows only on fields the server actually supplies: `org_id` (tenant, `workOrderTenantOk` — untagged passes, tagged must match the viewer, and a viewer with no `org_id` fails closed on admin/managed-vertical surfaces while ordinary own-only responses still render) and a canonical vertical (`workOrderVertical` reads `job_type`, else `vertical`, else nested `jobs.type` — and nothing else; a bare `type` on a work-order row is a document/work-order kind in this codebase, never a job vertical, so it is deliberately not in the chain). Both are **optional today**: an order with no canonical vertical is kept rather than dropped, so a response without `job_type` degrades to the existing server-authorized own-only list instead of an empty hub, while managed cross-job access stays fail-closed through the tenant guard and the server's own scoping. For the managed-fencing hub to be authoritative rather than best-effort, the backend must return an explicit per-row `org_id` and one of those canonical vertical fields, or expose tenant/vertical-scoped `my_work_orders` parameters.

### Pay tab — Work Order mode labour lines (reconciliation and direct billing)
- A WO-mode job card on the Pay tab invoice builder carries `wo_allocated` plus structured deduct lines (`wo_labour_lines` and `wo_lump_lines`). Hourly crew lines stay `{trade_name, hours, rate, amount}`. Work-order amounts Henry pays other trades are `{trade_name, line_kind: 'wo_pass_through', amount}` (prefilled from `negative_charges` when the job-centric builder hydrates `my_work_orders`). Freeform job deducts are `{description, amount, line_kind: 'lump_sum'}`. Hours-mode cards on the same builder expose the same `{description, amount}` lines next to the hours fields — any job-centric trade can add them; they submit as negative `extra_items` (`source: 'invoice_final_deduction'`) plus `final_deductions`, not as `wo_lump_lines` on a work-order extra. The holder's payable line is always **net = WO allocated − Σ deducts**; hourly prose stays `WO $559.5 − labour [Tendo 11.5h×$25=$287.5]=net $272`, pass-throughs add `− WO trades [Israel $40]`, and lump sums add `− other [Materials $10]`.
...
- Regression guard: `scripts/test-trade-wo-labour-lines.js` (runs in PR CI) executes the `[JC-PAYLOAD-BUILD-START..END]` block and pins the captain's reported case (WO $559.50, Tendo 11.5h×$25, net $272) plus multi-labourer, cleaning, attribution, Hours-card lump extras (hours+lump, lumps-only, incomplete lump), and validation behaviour. The backend counterpart in secureworks-backend pins reconciliation storage without automatic payout creation.
...
### Calendar
- `caFetchCalendarModel` routes on `NC.type`: make-safe reads only the `makesafe_board` feed (it no longer touches `api('calendar')`), every other vertical (fencing, patio, … and `all` for every one of them at once) crosses the strict `TradeCalendarSource` adapter (`// <trade-calendar-source>`, loader at `// <trade-calendar-source-all>`; multi-vertical contract owned by `AGENTS.md` → "Trade visibility & the manager view"), and repair reads the raw `api('calendar')` feed directly through `caFetchRepairCalendarModel`, keeping only repair-vertical rows — never the make-safe board, which excludes repair-family jobs. The Repair classification rule (`tradeJobVertical`, family-first) is owned by `AGENTS.md` → "Repair vertical".
- The `TradeCalendarSource` transport consumes exactly one published contract: `GET ops-api?action=trade_calendar` with `from`, `to`, `mode` (`mine` | `all`) and `type=<vertical>` (`fencing`, `patio`, …), with `type` OMITTED for `all`, JWT-authenticated through `api()` with `{ preserveSessionOnAuthFailure: true }`. `adaptV1` hard-rejects a payload whose `schema` is not `trade-calendar.v1`, whose effective `mode` is neither `mine` nor `all`, whose `type` or any event's raw `job_type` falls outside the requested vertical (for `all` the payload `type` must be null and the per-event check is skipped, since one payload legitimately mixes every vertical), or which omits `events[]` / `truncated`. `truncated: true` paints an explicit "reached the server limit" warning above the calendar. With no source registered the host renders a `data-calendar-state="contract-pending"` state rather than guessing rows.
- Mine/Everyone `scope` is a presentation lens for make-safe, but for every `TradeCalendarSource` vertical (fencing, patio, all) it is sent as the authorized `mode` and the server's effective mode wins (`NC.scope` is re-synced from the response). The model is keyed by user + vertical + lens (`NC.modelKey`), so a lens or trade switch refetches instead of repainting the previous payload. Trade chips and the Everyone chip are owned by `AGENTS.md` → "Trade visibility & the manager view".
- Crew-axis and job-axis modes, each at Day / Week / Month scale, plus a run sheet. Job-based **Today** is the boot default (`NC = { calView: 'cal', scale: 'day', axis: 'jobs', ... }`). Default vertical/scope and Reset are owned by `AGENTS.md` → "Trade own work".
...
- The My Jobs lens (`#adminJobToggle`; Everyone label owned by `AGENTS.md` → ACCESS IS NAMED) is offered to dispatchers (`admin` / `ops_manager`) and to managed-vertical leads; ops managers and managed leads open on Everyone, global admins still opt in. It only chooses the `mode` sent to `my_jobs` — an authorization request the backend answers with its own tenant/vertical set, never a client-side widening. An ordinary installer gets no toggle and stays own-only. Managed-lead Everyone scope, own-row merge and search-card chip/open rules are owned by `AGENTS.md` → "Trade visibility & the manager view". The open MakeSafe pool fallback (`loadMakesafePoolJobs`) remains dispatcher-only, and for a fencing lead the open-pool section is labelled **Fencing Ready for Crew** rather than Open MakeSafe Jobs.
...
| `trade_calendar` | GET | Field calendar rows, contract `trade-calendar.v1`; params `from`, `to`, `mode=mine\|all`, `type=<vertical>` (omit `type` for every vertical the viewer may see, returned with `type: null`) |
