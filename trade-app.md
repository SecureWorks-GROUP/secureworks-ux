...
### Calendar
- `caFetchCalendarModel` routes on `NC.type`: make-safe reads only the `makesafe_board` feed (it no longer touches `api('calendar')`), fencing crosses the strict `TradeCalendarSource` adapter (`// <trade-calendar-source>`), and repair reads the raw `api('calendar')` feed directly through `caFetchRepairCalendarModel`, keeping only repair-vertical rows — never the make-safe board, which excludes repair-family jobs. The Repair classification rule (`tradeJobVertical`, family-first) is owned by `AGENTS.md` → "Repair vertical".
...
- Mine/Everyone `scope` is a presentation lens for make-safe, but for fencing it is sent as the authorized `mode` and the server's effective mode wins (`NC.scope` is re-synced from the response). The model is keyed by user + vertical + lens (`NC.modelKey`), so a lens or trade switch refetches instead of repainting the previous payload, and the Everyone chip is hidden from a viewer who is neither a dispatcher nor a managed-vertical lead. The Trade chip list always offers Make-safe and Repair; a managed vertical such as Fencing is offered only to a viewer who manages it.
