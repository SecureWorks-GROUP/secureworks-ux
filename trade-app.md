...
- Mine/Everyone `scope` is a presentation lens for make-safe, but for every `TradeCalendarSource` vertical (fencing, patio, all) it is sent as the authorized `mode` and the server's effective mode wins (`NC.scope` is re-synced from the response). The model is keyed by user + vertical + lens (`NC.modelKey`), so a lens or trade switch refetches instead of repainting the previous payload, and the Everyone chip is hidden from a viewer who is neither a dispatcher nor a managed-vertical lead. The Trade chip list always offers Make-safe and Repair; Fencing is offered only to a viewer who manages it, and Patio / All to a viewer who manages that vertical or is a dispatcher (`_isAdmin`) — gating rule owned by `AGENTS.md` → "Trade visibility & the manager view".
