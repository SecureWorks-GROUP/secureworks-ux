...
- Columns are **Ready / Scheduled / On site / Done / Attention / Cancelled**. A dead or finished job status is terminal (Cancelled / Done) whatever the assignment says; an unrecognised assignment status lands in **Attention** and is counted in that vertical's own unmapped-job banner — never hidden, never silently placed in the wrong column.
